...
const ResolutionDetails = () => {
    ...
    let handelClose=()=>{
        axios.patch(`http://127.0.0.1:8000/helpdesk/department_ticket/${data.id}/`,{'resolved': true,'assign_to':user?.user_id},config).then(()=>
        {
        getApidata();
    ...
    let getApidata = () => {
        ...
        axios.get(`http://127.0.0.1:8000/helpdesk/department_ticket/${uid}`, config).then((response) => {
...

            {
                data && (<div>
                    ...
                    <div className='row'>
                        ...
                        <div className='col-8'>
                            {data.notes && data.notes.length > 0 ? (<table><thead><tr><th>Date</th><th>Comment by</th><th colSpan="1">Comment</th><th>Attachment</th></tr></thead>
                                <tbody>
                                    {data.notes.map((r) => <tr key={r.id}><td>{r.time.substr(0, 19)}</td><td>{r.username}</td><td colSpan="1">{r.text}</td><td>{r.attachment ? <a href={"http://127.0.0.1:8000" + r.attachment}>Download</a>:""}</td></tr>
                        ...
                        <textarea id="comment" style={{width:'100%'}} ref={ref} name="comment" type="text" placeholder='Add Comment'/><br/><input id="myattachment" ref={ref1} name="myattachment" type="file" />
                      <button onClick={()=>handelSubmit()} className='float-end'>Reply</button>  
                        </div>
